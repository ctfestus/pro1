...
// GET /api/resume
// Returns resume-builder.zip containing:
// resume-builder/SKILL.md -- the Claude skill
// resume-builder/resume-data.json -- the verified record used to build a resume.
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const { data: pathRows } = pathIds.length
      ? await supabase.from('learning_paths').select('id, title, description').in('id', pathIds)
      : { data: [] };
    ...
    const skillContent = readFileSync(join(process.cwd(), 'lib', 'resume-skill.md'), 'utf8')
      .replaceAll('{{PLATFORM_NAME}}', platformName || 'the platform');
    const dataContent = JSON.stringify(data, null, 2);

    const zip = new JSZip();
    const folder = zip.folder('resume-builder')!;
    folder.file('SKILL.md', skillContent);
    folder.file('resume-data.json', dataContent);
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        'Content-Disposition': 'attachment; filename="resume-builder.zip"',
      },
    });
  } catch (err) {
    console.error('[/api/resume]', err);
    return NextResponse.json({ error: 'Failed to generate resume package' }, { status: 500 });
  }
}
