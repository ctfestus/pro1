  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <AiReviewUpgradeNote accentColor={accentColor} upgradeUrl={upgradeUrl} />
      </div>
    </div>
  );
}
 ...
 * the middle of something they are reading. It stays generic rather than naming a plan, so these
 * surfaces do not each have to fetch the entitlement just to render a link.
 */
export function AiReviewUpgradeNote({
  ...
  return (
    ...
    >
      Upgrade for more AI reviews <ArrowUpRight className="h-3 w-3" />
    </a>
  );
}
...
export default function AiReviewUpgradePrompt({
  ...
  const label = planName ? `Upgrade to ${planName}` : 'Upgrade';
  ...
  return (
    <div className="flex max-w-sm flex-col items-center gap-3 px-5 py-6 text-center">
      ...
      <a
        ...
        className="mt-1 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold transition-all hover:opacity-90 active:scale-[0.98]"
