...
const EventCard = ({ item, past: isPast, index, C, imgErrors, onImageError }: { item: any; past?: boolean; index: number; C: typeof LIGHT_C; imgErrors: Set<string>; onImageError: (id: string) => void }) => {
...
const EventCarousel = ({ title, items, past: isPastRow, C, imgErrors, onImageError }: { title: string; items: any[]; past?: boolean; C: typeof LIGHT_C; imgErrors: Set<string>; onImageError: (id: string) => void }) => {
  ...
  return (
    <section className="rounded-2xl p-5 sm:p-6" style={{ background: C.card }}>
      ...
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-1 mt-4 snap-x items-start"
        ...
        {items.map((item, i) => <EventCard key={item.id} item={item} past={isPastRow} index={i} C={C} imgErrors={imgErrors} onImageError={onImageError} />)}
      </div>
    </section>
  );
};
...
export function EventsSection({ userId, C }: { userId: string; C: typeof LIGHT_C }) {
  ...
  return (
    <div className="space-y-6">
      {upcoming.length > 0 && <EventCarousel title="Upcoming" items={upcoming} C={C} imgErrors={imgErrors} onImageError={id => setImgErrors(prev => new Set(prev).add(id))} />}
      {past.length > 0 && <EventCarousel title="Past" items={past.slice().reverse()} past C={C} imgErrors={imgErrors} onImageError={id => setImgErrors(prev => new Set(prev).add(id))} />}
    </div>
  );
}
